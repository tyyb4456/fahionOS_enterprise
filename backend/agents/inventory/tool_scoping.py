"""
The shopify-mcp server is shared across every brand, so each of its tools
takes `brand_id` as an explicit first parameter (see mcp_servers/shopify_server
/server.py). We don't want the LLM filling that in itself — a prompt
injection buried in a product description or return note could otherwise
steer a tool call at another tenant's store. Instead we bind the current
run's brand_id in a closure and strip the parameter from the schema the
model actually sees.
"""
